...
public static class SettingDocs
{
    public static string MechanismFor(string settingId)
    {
        ...
        return settingId switch
        {
            ...
            "vbs" => @"HKLM\SYSTEM\...\Control\DeviceGuard {EnableVirtualizationBasedSecurity, RequirePlatformSecurityFeatures, Mandatory, HypervisorEnforcedCodeIntegrity}  +  DeviceGuard\Scenarios\*\Enabled (every scenario subkey)  +  Control\Lsa\LsaCfgFlags  +  HKLM\SOFTWARE\Policies\Microsoft\Windows\DeviceGuard {EnableVirtualizationBasedSecurity, LsaCfgFlags, HypervisorEnforcedCodeIntegrity} (all DWORD; explicit 0 = disabled; reboot required)",
    ...
    public static string ApplyCommandFor(string settingId, string rawDesired = "")
    {
        ...
        return settingId switch
        {
            ...
            "vbs" => @"# Disable the full VBS stack (explicit zeros survive feature updates; reboot required):" + "\n" +
                     ...
                     @"foreach($s in 'HypervisorEnforcedCodeIntegrity','CredentialGuard','SystemGuard','KernelShadowStacks','WindowsHello'){ $k=""$dg\Scenarios\$s""; New-Item $k -Force | Out-Null; Set-ItemProperty $k -Name Enabled -Value 0 -Type DWord }; " +
                     @"foreach($m in 'WasEnabledBy','EnabledBootId','ChangedInBootCycle'){ Remove-ItemProperty ""$dg\Scenarios\HypervisorEnforcedCodeIntegrity"" -Name $m -EA SilentlyContinue }; " +
                     @"Set-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' -Name LsaCfgFlags -Value 0 -Type DWord; " +
                     @"$p='HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeviceGuard'; New-Item $p -Force | Out-Null; foreach($n in 'EnableVirtualizationBasedSecurity','LsaCfgFlags','HypervisorEnforcedCodeIntegrity'){ Set-ItemProperty $p -Name $n -Value 0 -Type DWord }   # reverse: set EnableVirtualizationBasedSecurity=1, HVCI Enabled=1 + WasEnabledBy=2, Remove-ItemProperty the policy values and LsaCfgFlags",
    ...
    public static string VerifyCommandFor(string settingId)
    {
        ...
        return settingId switch
        {
            ...
            "vbs" => @"# Run elevated. Configured state (registry):" + "\n" +
                     @"$dg='HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard'; @{EVBS=(Get-ItemProperty $dg -Name EnableVirtualizationBasedSecurity -EA SilentlyContinue).EnableVirtualizationBasedSecurity; LsaCfg=(Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' -Name LsaCfgFlags -EA SilentlyContinue).LsaCfgFlags; Scenarios=(Get-ChildItem ""$dg\Scenarios"" -EA SilentlyContinue | ForEach-Object { ""$($_.PSChildName)=$((Get-ItemProperty $_.PSPath -Name Enabled -EA SilentlyContinue).Enabled)"" }) -join ', '}" + "\n" +
                     @"# Runtime truth (0 = VBS not running; only changes after a reboot):" + "\n" +
